...
description = "Tuning RelaySum/Model with and without momentum with randomly split data according to different values of alpha"
...
for alpha in [1, 0.1, 0.01]:
    for topology in ["chain", "double-binary-trees"]:
        corr = {"chain": 5, "double-binary-trees": 3}[topology]
        # gossip with momentum
        lr_for_centralized = 0.1
        lrs = [lr_for_centralized / 2 * corr, lr_for_centralized * corr, lr_for_centralized * corr * 2]
        if alpha < 1:
        ...
        lr_for_centralized = 0.2
        for lr in [lr_for_centralized / 2 * corr, lr_for_centralized * corr, lr_for_centralized * corr * 2]:
            config = {**base_config, "learning_rate": lr, "momentum": 0.0, "topology": topology, "non_iid_alpha": alpha}
            job_name = "alpha{non_iid_alpha}-{algorithm}-{topology}-mom{momentum}-lr{learning_rate}".format(**config)
            if mongo.job.count_documents({"job": job_name, "experiment": experiment, **{f"config.{key}": value for key, value in config.items()}}) > 0:
                # We have this one already
                continue
