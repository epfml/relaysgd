...
description = "Tuning all-reduce SGD with and without momentum with randomly split data"
...
for lr in [0.05, 0.1, 0.2]:
    ...
    job_name = "{algorithm}-mom{momentum}-lr{learning_rate}".format(**config)
...
for lr in [0.1, 0.2, 0.4, 0.8, 1.6]:
    config = {**base_config, "learning_rate": lr, "momentum": 0.0}
    job_name = "{algorithm}-mom{momentum}-lr{learning_rate}".format(**config)
    if mongo.job.count_documents({"job": job_name, "experiment": experiment, **{f"config.{key}": value for key, value in config.items()}}) > 0:
        # We have this one already
        continue
