...
description = "Tuning PushSum with exponential graph with and without momentum with randomly split data according to different values of alpha"
...
for alpha in [1, 0.1, 0.01]:
    ...
    lrs = [lr_for_centralized / 2, lr_for_centralized, lr_for_centralized * 2, lr_for_centralized * 4]
    if alpha < 1:
        lrs.append(lr_for_centralized / 4)
    for lr in lrs:
        config = {**base_config, "learning_rate": lr, "momentum": 0.9, "non_iid_alpha": alpha}
    ...
    for lr in [lr_for_centralized / 4, lr_for_centralized / 2, lr_for_centralized, lr_for_centralized * 2]:
        config = {**base_config, "learning_rate": lr, "momentum": 0.0, "non_iid_alpha": alpha}
        job_name = "alpha{non_iid_alpha}-{topology}-mom{momentum}-lr{learning_rate}".format(**config)
        if mongo.job.count_documents({"job": job_name, "experiment": experiment, **{f"config.{key}": value for key, value in config.items()}}) > 0:
            # We have this one already
            continue
